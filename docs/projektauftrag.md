...
## 10f. Taktik-Taxonomie-Nachpflege: Alias-Mechanismus *(neu, v10, Planungsstand — noch nicht umgesetzt)*
...
**Design-Entscheidung: stabile interne `tactic.id`, entkoppelt von MITREs aktuellem STIX-`phase_name`, über eine kuratierte Alias-Liste.** Eine Alternative wäre, `tactic.id` bei jeder MITRE-Umbenennung mitzuändern (echte Umbenennung der Primärschlüssel-Slug) — das würde aber eine FK-Kaskade über `technique.tactic_id` und `tactic_default_mapping.tactic_id` auslösen und bricht bei jeder künftigen Umbenennung erneut. Stattdessen: `tactic.id` bleibt ein stabiler interner Bezeichner (wie bisher), nur `tactic.name` folgt MITREs aktueller Bezeichnung. Ein neuer, kuratierter Crosswalk `TACTIC_PHASE_ALIASES` (Python-Dict, analog `MITIGATION_CROSSWALK` in `seed_data.py` — kein Admin-UI, von einem Entwickler erweitert, wenn MITRE erneut umbenennt) bildet STIX-`phase_name` auf unsere `tactic.id` ab:

```
TACTIC_PHASE_ALIASES = {
    # ... die 13 unveränderten 1:1-Zuordnungen ...
    "stealth": "defense-evasion",        # MITRE-Umbenennung, unsere id bleibt stabil
    "defense-impairment": "defense-impairment",  # echte neue 15. Taktik
}
```

**Konkrete Schritte (noch nicht umgesetzt):**
1. Migration: neue `tactic`-Zeile `id="defense-impairment"`, `name="Defense Impairment"`; `tactic.name` für `id="defense-evasion"` auf `"Stealth"` aktualisieren; `mitre_order` für die Taktiken 7-14 um eins verschieben, `defense-impairment` erhält Order 7 (an der bisherigen Position von Defense Evasion/Stealth in der Kill-Chain) — unkritisch, da `mitre_order` nur für Sortierung und die "früheste Taktik gewinnt"-Fallback-Regel verwendet wird, nicht als stabiler externer Bezeichner.
2. `tactic_default_mapping` für die neue Taktik `defense-impairment` anlegen (Taktik-Standardmapping, damit jede Technik dieser Taktik mindestens eine Basis-Empfehlung hat — Abschnitt 5-Invariante).
3. `app/services/mitre_import.py::_resolve_tactic_id()` erweitern: erst gegen `TACTIC_PHASE_ALIASES` auflösen, Fallback auf direkten `Tactic.id`-Lookup (für den Fall, dass MITRE künftig doch wieder mit unseren Slugs übereinstimmt).
4. `frontend/src/lib/tactics.ts`: `TACTIC_NAMES` um `"Stealth"` (statt `"Defense Evasion"`) und `"Defense Impairment"` ergänzen — 15 statt 14 Einträge; jede Doku-Stelle, die pauschal "14 Taktiken" sagt, prüfen und ggf. auf "aktuell 15" korrigieren.
5. **Kein neuer Import-Code für die "Wiederherstellung" nötig** — der in Schritt 6 gebaute Diff-Mechanismus deckt das bereits ab: sobald Schritt 3 die Alias-Auflösung kennt, erkennt ein erneuter MITRE-Import (`POST /fetch` oder `/upload`) für jede der ~204 aktuell unmappbaren Techniken automatisch die korrekte `tactic_id` — sichtbar im bestehenden `updated_techniques`-Diff, vom Admin wie gewohnt selektiv zu übernehmen. "Mapping wiederherstellen" und "neues Mapping durchführen" sind damit derselbe, bereits vorhandene Vorgang, keine zusätzliche Funktion.

**Zukunftssicherheit:** Die nächste MITRE-Umbenennung braucht künftig nur einen neuen Eintrag in `TACTIC_PHASE_ALIASES` (plus ggf. eine neue `tactic`-Zeile bei einer echten Taxonomie-Erweiterung) — keine erneute Konzeptarbeit, kein FK-Kaskaden-Risiko.
...
10. ✅ *(Grundsatzentscheidung getroffen, v10 — Umsetzung offen)* MITRE hat "Defense Evasion" (TA0005) in **"Stealth"** umbenannt und **"Defense Impairment" (TA0112)** als neue 15. Taktik eingeführt. **Entscheidung: Breakpoint orientiert sich immer an der aktuellen MITRE-Taxonomie** — dauerhafte Leitplanke, nicht nur für diesen einen Fall. Konkreter Mechanismus (stabile interne `tactic.id`, Umbenennung/neue Taktik über einen kuratierten Alias-Crosswalk statt FK-Kaskade) in Abschnitt 10f entworfen, **noch nicht implementiert**.

Diese Fragen blockieren Schritt 1 nicht zwingend (sinnvolle Defaults sind oben angegeben), sollten aber vor Schritt 5 (LLM-Anbindung) und vor einem produktiven Deployment final geklärt sein. Fragen 2 und 5 sollten idealerweise **vor** dem ersten Commit geklärt sein, da sie sich sonst flächendeckend (Formatierung, Dockerfiles) niederschlagen und später aufwändig nachgezogen werden müssten. Frage 10 ist konzeptionell geklärt, die Umsetzung (Abschnitt 10f) sollte vor dem nächsten produktiven MITRE-Import erfolgen, blockiert aber nichts akut (die Diff-Ansicht behandelt unmappbare Techniken bereits transparent, statt sie stillschweigend falsch zuzuordnen).
