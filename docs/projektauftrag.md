...
> **v13 — ergänzt am 01.09.2026.** Abschnitt 10h (VPS-Deploy) umgesetzt: `frontend/Dockerfile` zu Multi-Stage-Build umgebaut (Dev-Stage unverändert, Produktions-Stage baut mit `npm run build` und liefert über `nginx:1.27-alpine` aus statt des Vite-Dev-Servers), neues `infra/docker-compose.prod.yml` (nur noch nginx öffentlich erreichbar, `db`/`backend` ausschließlich intern), `frontend/nginx.conf` proxied `/api/` zum Backend, `frontend/.env.production` macht den Produktions-Build domain-portabel (keine hartkodierte `127.0.0.1:8000`-Referenz mehr im Bundle). **Bewusste, vom Auftraggeber freigegebene Ausnahme** von der "nicht verhandelbar"-Guardrail aus Abschnitt 8: der VPS-Betrieb ist explizit nur als POC ohne echte Kundendaten freigegeben, solange kein OIDC-Auth-Layer existiert — Details und Einschränkung in 10h. Vollständige Suite nach dem Umbau erneut grün. Frühere Änderungen: ▶ **Update v12** — Abschnitt 10g (vollständige App-Review auf Bugs & Flaws) umgesetzt: 9 echte Bugs gefunden, behoben und mit Regressionstests abgesichert (u. a. drei verschiedene FK-Verletzungsfälle beim selektiven MITRE-Import, ein Stale-Closure-Bug bei Engagement-Wechsel, unvalidierter `ref`-Parameter in der GitHub-Fetch-URL, deprecated/inaktive Zeilen ohne UI-Zugriff), ein Fund als bewusste Design-Entscheidung erkannt statt "gefixt", ein seltener Edge-Case als bekannte Grenze dokumentiert. Vollständige Suite grün (96 Backend-, 16 Frontend-Tests, ruff/tsc/oxlint sauber, Build erfolgreich). ▶ **Update v11** — Abschnitt 10f (Taktik-Taxonomie-Alias-Mechanismus) umgesetzt: "Defense Evasion" zeigt jetzt als "Stealth" (stabile interne `tactic.id` bleibt `defense-evasion`), neue 15. Taktik "Defense Impairment" ergänzt, `TACTIC_PHASE_ALIASES`/`TACTIC_NAME_OVERRIDES` in `seed_data.py`, `_resolve_tactic_id()` löst darüber auf. Bewusste Vereinfachung ggü. dem ursprünglichen Plan: keine Migration nötig, der ohnehin bei jedem Start laufende idempotente Re-Seed synchronisiert Name/neue Taktik selbstheilend. 5 neue + 3 angepasste Backend-Tests, im Browser gegen laufendes Backend verifiziert (Taktik-Dropdown, Filter, echter Import-Diff). ▶ **Update v10** — fünf offene Fragen aus Abschnitt 12 vom Auftraggeber entschieden: LLM-Plattform-Anbindung zurückgestellt (Frage 1), Auth-Provider = internes System mit OIDC + SCIM (Frage 3), keine Löschpflicht in Breakpoint selbst — Rohdaten liegen extern (Frage 8), ein Admin reicht für MITRE-Import-Freigabe, kein Vier-Augen-Prinzip (Frage 6), Breakpoint orientiert sich immer an der aktuellen MITRE-Taxonomie (Frage 10). ▶ **Update v9** — Schritt 6 (MITRE-Import + Mitigations-Bootstrap, Abschnitt 10e) umgesetzt, kritisch geprüft (zwei echte Bugs gefunden und behoben, siehe 10e.5) und dokumentiert; Basis waren echte, in der Session geprüfte STIX-Bundle-Daten statt Annahmen. ▶ **Update v8** — Schritt 5 (Sales-Briefing) umgesetzt, kritisch geprüft und Bugs behoben — Details in Abschnitt 10d.5 (DoD vollständig abgehakt inkl. Review-Ergebnis). ▶ **Update v7** — Abschnitt 10d konkretisierte Schritt 5 (Sales-Briefing/PydanticAI) im Vorfeld der Umsetzung: async ohne neue Infrastruktur (FastAPI `BackgroundTasks` statt Task-Queue, Grenzen dokumentiert), LLM-Anbindung als ungeklärte Annahme markiert (offene Frage 1 aus Abschnitt 12 bleibt offen), Post-Processing-Guard testbar ohne echte LLM-Anbindung dank PydanticAI `TestModel`/`FunctionModel`. ▶ **Update v6** — Abschnitt 6a.3 plant die Nutzung offizieller MITRE-Mitigations (M-Nummern) als Bootstrap für spezifische Mappings, inkl. eines konkreten Code-Funds (`resolve_technique()` schreibt `mapping_source` hartkodiert statt aus der DB zu lesen — muss vor Schritt 6 behoben werden). ▶ **Update v5** — Abschnitt 10c konkretisiert Schritt 4 (Portfolio-Modul). ▶ **Update v4** — Abschnitt 10b konkretisiert Schritt 3 (Frontend-Anbindung). ▶ **Update v3** — gezielte, enge Ausnahme von der Offline-Regel für den MITRE-Import (Abschnitt 2, 6a.2, 9). ▶ **Update v2** — aus der Review-Session vom 29.08.2026: kritische Prüfung des Auftrags + Code-Review des interaktiven HTML-Prototyps (`breakpoint-dashboard.html`) + eine vom Auftraggeber formulierte Zielbild-Zusammenfassung (siehe Abschnitt 2a).
...
## 10f. Taktik-Taxonomie-Nachpflege: Alias-Mechanismus *(v10 geplant, v11 umgesetzt)*

**Auslöser:** Abschnitt 10e.5 / Abschnitt 12 Frage 10 — MITRE hat "Defense Evasion" (TA0005) in "Stealth" umbenannt und mit "Defense Impairment" (TA0112) eine neue 15. Taktik eingeführt. **Grundsatzentscheidung des Auftraggebers:** Breakpoint orientiert sich **immer** an der jeweils aktuellen MITRE-Taxonomie — das ist kein Einmal-Fix für diese eine Umbenennung, sondern eine dauerhafte Leitplanke, die jede künftige MITRE-Umbenennung/Aufspaltung/Zusammenlegung von Taktiken ohne Sonderbehandlung abdecken muss.

**Design-Entscheidung: stabile interne `tactic.id`, entkoppelt von MITREs aktuellem STIX-`phase_name`, über eine kuratierte Alias-Liste.** Eine Alternative wäre, `tactic.id` bei jeder MITRE-Umbenennung mitzuändern (echte Umbenennung der Primärschlüssel-Slug) — das würde aber eine FK-Kaskade über `technique.tactic_id` und `tactic_default_mapping.tactic_id` auslösen und bricht bei jeder künftigen Umbenennung erneut. Stattdessen: `tactic.id` bleibt ein stabiler interner Bezeichner (wie bisher), nur `tactic.name` folgt MITREs aktueller Bezeichnung. Der kuratierte Crosswalk `TACTIC_PHASE_ALIASES` (`scripts/seed_data.py`, analog `MITIGATION_CROSSWALK` — kein Admin-UI, von einem Entwickler erweitert, wenn MITRE erneut umbenennt) bildet STIX-`phase_name` auf unsere `tactic.id` ab; für die 13 unveränderten Taktiken ist kein Eintrag nötig (Fallback `phase_name -> phase_name`), nur die tatsächlichen Abweichungen sind gelistet:

```python
TACTIC_PHASE_ALIASES = {
...
**Umgesetzt — mit einer bewussten Vereinfachung gegenüber der ursprünglichen Planung:** Statt einer Alembic-Migration übernimmt der bereits bestehende, bei jedem Container-Start laufende idempotente Re-Seed (`docker-entrypoint.sh` → `python -m scripts.seed`, s. Abschnitt 10e/6a) die komplette Datenpflege — keine Schema-Änderung nötig, da weder eine neue Spalte noch eine neue Tabelle erforderlich ist:

1. **`seed_tactics()` synchronisiert jetzt auch den Anzeigenamen** einer bereits existierenden `tactic`-Zeile (nicht mehr nur "anlegen, falls fehlt") — über `TACTIC_NAME_OVERRIDES = {"Defense Evasion": "Stealth"}`. Selbstheilend: eine bereits gesäte DB korrigiert den Namen beim nächsten Neustart automatisch, keine manuelle Migration/Downtime nötig.
2. **`TACTIC_GROUPS["Defense Impairment"] = []`** ans Ende angehängt (Abschnitt 10e.1 zurückgestellt: statt der ursprünglich geplanten Order-Verschiebung 7→8..15 einfach als Order 15 angehängt — keine der 14 bestehenden Taktiken muss umnummeriert werden, `mitre_order` ist ohnehin nur Sortier-/Tie-Break-Hilfe, kein stabiler externer Bezeichner). Wird beim nächsten Re-Seed automatisch als neue `tactic`-Zeile angelegt.
3. **`TACTIC_DEFAULTS["Defense Impairment"]`** ergänzt (gleiches Profil wie Defense Evasion/Stealth als Startwert — inhaltliche Nähe, MITRE beschreibt Defense Impairment als "Disable or modify security tools/logs"). Wird vom bestehenden `seed_tactic_defaults()` ohne Änderung übernommen.
4. **`app/services/mitre_import.py::_resolve_tactic_id()`** löst `phase_name` zuerst gegen `TACTIC_PHASE_ALIASES` auf (Fallback: `phase_name` selbst, deckt die 13 unveränderten Taktiken weiterhin ab).
5. **`frontend/src/lib/tactics.ts`**: `TACTIC_NAMES` zeigt jetzt `"Stealth"` (statt `"Defense Evasion"`) und ergänzt `"Defense Impairment"` — 15 statt 14 Einträge.
6. **Kein neuer Import-Code für die "Wiederherstellung" nötig, wie geplant** — der in Schritt 6 gebaute Diff-Mechanismus deckt das ab: ein erneuter MITRE-Import erkennt für jede zuvor unmappbare Technik jetzt automatisch die korrekte `tactic_id`, sichtbar im bestehenden `updated_techniques`-Diff. Im Browser gegen das Test-Fixture verifiziert: eine Technik mit `phase_name="stealth"` löst korrekt auf `tactic_id="defense-evasion"` auf, eine mit `phase_name="defense-impairment"` auf die neue Taktik.

**Verifiziert:** 5 neue Backend-Tests (Namens-Sync, Selbstheilung bei erneutem Re-Seed, neue Taktik + Taktik-Standardmapping, Alias-Auflösung in `compute_diff()`) plus Anpassung von drei bestehenden Tests (Taktik-Anzahl 14→15, Namensvergleich über `TACTIC_NAME_OVERRIDES`). Im echten Browser gegen laufendes Backend geprüft: Taktik-Filter-Dropdown zeigt "Stealth"/"Defense Impairment" korrekt, Filterung funktioniert, ein echter MITRE-Import-Diff löst beide Fälle sichtbar korrekt auf.

**Zukunftssicherheit:** Die nächste MITRE-Umbenennung braucht künftig nur einen neuen Eintrag in `TACTIC_PHASE_ALIASES`/`TACTIC_NAME_OVERRIDES` (plus ggf. eine neue `TACTIC_GROUPS`/`TACTIC_DEFAULTS`-Zeile bei einer echten Taxonomie-Erweiterung) — keine erneute Konzeptarbeit, keine Migration, kein FK-Kaskaden-Risiko.

---

## 10g. Vollständige App-Review: Bugs & Flaws *(v12 umgesetzt)*

**Auslöser:** expliziter Auftrag des Auftraggebers, die gesamte App (nicht nur den zuletzt umgesetzten Schritt) auf Bugs und Flaws zu prüfen. Vorgehen: automatisierte Code-Review über Backend und Frontend, jeder Fund einzeln gegen den echten Code/die echte Laufzeit verifiziert (nicht blind übernommen), echte Bugs behoben und mit Regressionstest abgesichert, ein Fund als bewusste Design-Entscheidung erkannt und **nicht** verändert, ein seltener Edge-Case als bekannte Grenze dokumentiert statt behoben.

**Gefundene und behobene Bugs:**

1. **`apply_batch()` — Mitigation-Kandidat ohne zugehörige Technik.** Die Diff-Ansicht erlaubt zwei unabhängige Checkbox-Gruppen ("neue Technik übernehmen" / "Mitigation übernehmen"). Wählte ein Admin nur die Mitigation aus, ohne die zugehörige neue Technik selbst zu übernehmen, verletzte das INSERT die FK-Constraint. Fix: Kandidaten ohne existierende Technik werden still übersprungen (konsistent mit dem Prinzip "selektive Übernahme", Abschnitt 10e.3).
2. **`apply_batch()` — INSERT-Reihenfolge bei neuen Eltern/Kind-Techniken.** `bundle.techniques` folgt der rohen STIX-Objektreihenfolge, die eine Sub-Technique vor ihrer neuen Eltern-Technik auflisten kann. Empirisch gegen echtes Postgres verifiziert: SQLAlchemy ordnet roh gesetzte Self-Referential-FK-Spalten (`parent_technique_id`) beim Flush **nicht** automatisch um, selbst mit konfigurierter `relationship(remote_side=[id])`. Fix: neue Techniken werden vor dem Insert per Tiefensuche (`_sort_new_techniques_by_dependency()`) topologisch sortiert.
3. **`apply_batch()` — Sub-Technique ohne ausgewählte neue Eltern-Technik.** Beim eigenen Live-Test gegen die laufende App über Fund 2 hinaus entdeckt: wurde nur die Sub-Technique (z. B. T9001.001), nicht aber ihre ebenfalls neue Eltern-Technik (T9001) ausgewählt, zeigte `parent_technique_id` auf eine nie angelegte Zeile — erneute FK-Verletzung, diesmal unabhängig von der Sortierreihenfolge. Fix: fehlt die Eltern-Technik sowohl in der Auswahl als auch bereits in der DB, wird die Sub-Technique trotzdem angelegt, nur ohne Eltern-Verknüpfung (fällt auf den Taktik-Standard zurück, bis ein künftiger Import die Eltern-Technik nachliefert).
4. **`ImportDiffView` behält Checkbox-Auswahl über Batch-Wechsel hinweg.** Fehlendes `key`-Prop führte dazu, dass beim Wechsel zwischen zwei Import-Batches der interne Auswahl-State der Komponente nicht zurückgesetzt wurde — ein Admin konnte versehentlich Technik-IDs aus Batch A gegen Batch B anwenden. Fix: `key={batch.id}` erzwingt Remount bei Batch-Wechsel.
5. **`TechniqueCard` zeigt `mitre_derived`-Mappings fälschlich als "Taktik-Standard".** Die Badge-Logik unterschied nur binär `specific` vs. "alles andere", obwohl `mapping_source` seit Schritt 6 drei Werte kennt. Fix: vollständige Badge-/Label-Zuordnung für alle drei `mapping_source`-Werte.
6. **Deprecated Techniken und deaktivierte Portfolio-Technologien waren aus der UI nicht mehr erreichbar.** Beide APIs filterten historische/deaktivierte Zeilen standardmäßig heraus, ohne UI-Möglichkeit, sie wieder einzublenden — problematisch, weil Breakpoint unbegrenzt vorhält (Abschnitt 12 Frage 8) und Admins historische Daten nachvollziehen können müssen. Fix: `include_deprecated`/`include_inactive`-Parameter plus "Deprecated anzeigen"/"Inaktive anzeigen"-Checkboxen in `Techniques.tsx`/`Portfolio.tsx`, mit entsprechenden Badges.
7. **Unvalidierter `ref`-Query-Parameter wird direkt in die GitHub-Raw-URL interpoliert.** `POST /api/mitre-import/fetch` (Admin-Endpoint, vor Schritt 7 ohne Auth-Gate) nahm `ref` ungeprüft entgegen und baute daraus die Fetch-URL — ein Pfad-Traversal-artiger Wert (`../../…`) hätte die URL manipulieren können. Fix: enges Whitelist-Pattern (`GIT_REF_PATTERN`), `InvalidGitRefError` bei Ablehnung, geprüft vor jeder Interpolation.
8. **`Engagements.tsx` — Stale-Closure-Bug in `findingsMutation.onSuccess`.** Die `onSuccess`-Callback griff auf äußeren Closure-State (`engagementId`) statt auf die tatsächlichen Mutation-Variablen zu — bei schnell aufeinanderfolgenden Engagement-Wechseln konnte die Query-Invalidierung das falsche Engagement treffen. Fix: `onSuccess: (_data, variables) => …` nutzt `variables.engagementId` statt äußerem State.
9. **`parse_bundle()` filtert revoked/deprecated STIX-Relationship-Objekte nicht.** Der Deprecation-Check lief nur auf den referenzierten `attack-pattern`/`course-of-action`-Objekten, nicht auf der `relationship`-Objekt selbst — eine als `revoked`/`x_mitre_deprecated` markierte `mitigates`-Relationship (auch wenn Quelle/Ziel aktiv sind) wurde fälschlich übernommen. Fix: zusätzliche Prüfung direkt auf dem Relationship-Objekt.

**Bewusst nicht verändert:** ein Fund zur Sichtbarkeit von `flagged_for_review`-Inhalten widerspricht der expliziten Design-Entscheidung aus Abschnitt 10d (Post-Processing-Guard zeigt geflaggte Inhalte bewusst statt sie zu verstecken) — kein Bug, sondern Absicht.

**Als bekannte Grenze dokumentiert, nicht behoben:** eine `technique_id`-Umbenennung durch MITRE (Abkopplung von der `stix_id`-Historie) ist ein seltener, komplexer Edge-Case, der über den Rahmen dieser Review-Runde hinausgeht und separat geplant werden sollte, falls er in der Praxis auftritt.

**Verifiziert:** 6 neue Backend-Regressionstests (`test_mitre_import_service.py`) plus 3 neue Frontend-Regressionstests (`Analyzer.test.tsx`, `Portfolio.test.tsx`, `Techniques.test.tsx`) für die konkreten Fehlerszenarien. Vollständiger Durchlauf nach allen Fixes: 96/96 Backend-Tests (`pytest`), 16/16 Frontend-Tests (`vitest`), `ruff check` sauber auf allen geänderten Dateien, `tsc --noEmit` und `oxlint` sauber, Produktions-Build erfolgreich. Fund 3 zusätzlich live im Browser gegen die laufende App reproduziert (echter `IntegrityError` im Uvicorn-Log) und nach dem Fix über den Regressionstest gegen eine echte Postgres-Session (mit tatsächlich erzwungenen FK-Constraints) abgesichert.

---

## 10h. VPS-Deploy (Produktions-Umbau) *(v13 umgesetzt)*

**Auslöser:** Auftraggeber will die App auf einem VPS als Proof-of-Concept betreiben können. Rückfrage zum Zugriffsschutz, solange der OIDC-Auth-Layer (Schritt 7, Abschnitt 12 Frage 3) noch nicht existiert, wurde vom Auftraggeber explizit beantwortet: **"wird nur als POC aufgebaut, kein besonderer Schutz notwendig."**

**Wichtig — bewusste, dokumentierte Ausnahme von der Schritt-1-Guardrail (Abschnitt 8):** die dort als "nicht verhandelbar" beschriebene Regel (nur `localhost`/Loopback-Bindung, solange kein Auth-Layer existiert) gilt für den **produktiven** Betrieb mit echten Kundendaten. Der hier umgesetzte VPS-Betrieb ist ausdrücklich als **POC ohne echte Kundendaten** freigegeben — eine bewusste, vom Auftraggeber getroffene Ausnahme für diesen einen Zweck, keine Aufhebung der Guardrail selbst. Vor Nutzung mit echten Findings/Engagements muss entweder der OIDC-Layer stehen oder ein Zugriffsschutz (VPN/IP-Allowlist/Basic-Auth) vorgeschaltet werden.

**Zwei konkrete Produktions-Lücken behoben (aus der Antwort auf "welche Systemvoraussetzungen für ein VPS hat die App"):**

1. **`frontend/Dockerfile` startete den Vite-*Dev*-Server statt eines Produktions-Builds.** Umgebaut zu einem Multi-Stage-Build: Stage `dev` (unverändertes Verhalten für `infra/docker-compose.yml`, `npm run dev` gegen bind-gemountete Quellen) und Stage `build` → `production` (`npm run build`, danach `nginx:1.27-alpine` liefert die statischen Assets aus). Kein Node-Runtime-Overhead mehr im produktiven Betrieb, echtes Minifying/Caching.
2. **Alle Services banden nur an `127.0.0.1`** (Schritt-1-Guardrail) — für einen VPS muss zwingend genau ein Einstiegspunkt öffentlich erreichbar sein, der Rest nur intern. Neues `infra/docker-compose.prod.yml`: `db` und `backend` publizieren **keinen** Host-Port mehr (nur noch über das interne Docker-Netzwerk erreichbar), einzig der `frontend`/nginx-Service bindet an `${PUBLIC_HTTP_PORT:-80}` auf allen Interfaces. `nginx.conf` liefert die gebauten Assets aus (mit SPA-Fallback für `react-router-dom`) und reverse-proxied `/api/` zum `backend`-Service — dieselbe Origin für Frontend und API, kein CORS-Preflight im produktiven Pfad nötig.

**Zusätzlich für Portabilität behoben:** `frontend/src/lib/api.ts` hatte `http://127.0.0.1:8000` als API-Base-URL fest verdrahtet (Fallback, wenn `VITE_API_BASE_URL` nicht gesetzt ist) — das ist zur Build-Zeit in den JS-Bundle eingebacken (`import.meta.env`, Vite). Ein auf einem VPS gebautes Image hätte also weiterhin `127.0.0.1:8000` aufgerufen und wäre nutzlos gewesen. Neu: `frontend/.env.production` setzt `VITE_API_BASE_URL=""` (wird von `vite build` automatisch geladen) → alle Requests werden relativ (`/api/...`) und laufen über den nginx-Proxy, unabhängig von Domain/IP des VPS. Lokale Entwicklung (`npm run dev`, `.env.production` wird dabei nicht geladen) verhält sich unverändert. Verifiziert: gebauter Bundle enthält nachweislich keine `127.0.0.1:8000`-Referenz mehr (`grep` gegen `dist/assets/index-*.js`).

**Weitere kleinere Anpassungen:**
- `backend/docker-entrypoint.sh`: `--reload` (Datei-Watcher, nur für lokale Entwicklung mit gemounteten Quellen sinnvoll) ist jetzt über `UVICORN_RELOAD` steuerbar; `infra/docker-compose.prod.yml` setzt `UVICORN_RELOAD=false`.
- `.env.example`: `PUBLIC_HTTP_PORT` dokumentiert (Standard 80).

**Verifiziert:** `docker compose -f infra/docker-compose.prod.yml config` und `docker compose -f infra/docker-compose.yml config` validieren beide fehlerfrei (Docker-Daemon in dieser Umgebung nicht erreichbar, daher kein echter `up --build` möglich — analog zur bereits in Schritt 1 dokumentierten Einschränkung, s. README "Stand Schritt 1"). Produktions-Build (`npm run build`) läuft durch, gebauter Bundle enthält keine hartkodierte `127.0.0.1`-Referenz mehr. Vollständige Suite nach dem Umbau erneut grün: 96 Backend-Tests, 16 Frontend-Tests, `tsc`/`oxlint` sauber. **Vor dem ersten echten VPS-Einsatz bitte einmal `docker compose -f infra/docker-compose.prod.yml up --build` in einer Umgebung mit Docker-Hub-Zugriff gegentesten** (gleiche Einschränkung wie beim lokalen Compose-Setup aus Schritt 1).

**Deploy-Kurzanleitung:** siehe README.md, Abschnitt "VPS/Produktions-Deploy".
...
1. ✅ *(beantwortet, v10)* Wie wird die interne LLM-Plattform angesprochen? **Zurückgestellt** — der Auftraggeber verschiebt diese Entscheidung bewusst; Schritt 5 bleibt auf der dokumentierten Annahme (OpenAI-kompatible API) stehen und schlägt ohne konfigurierte `LLM_PLATFORM_BASE_URL` weiterhin klar fehl statt auf einen externen Anbieter auszuweichen. Vor einem produktiven Einsatz der Sales-Briefing-Funktion muss diese Frage doch noch final geklärt werden.
...
3. ✅ *(beantwortet, v10)* Welcher Auth-Provider ist im Datacenter Standard? **Internes System mit OIDC + SCIM** (SCIM für automatisierte Nutzer-/Rollen-Provisionierung). Bestimmt die konkrete Umsetzung des Rollenmodells in Schritt 7 — PydanticAI/FastAPI-seitig ein Standard-OIDC-Client (Authorization-Code-Flow gegen den internen Identity Provider), Nutzer-/Rollensynchronisation über SCIM statt manueller Pflege.
...
6. ✅ *(beantwortet, v10; v3 teilweise beantwortet)* MITRE selbst veröffentlicht ca. zweimal jährlich (Frühjahr/Herbst) — ein quartalsweiser "Prüfen"-Klick durch einen Admin deckt das komfortabel ab. **Ein einzelner Admin reicht zur Freigabe** — kein Vier-Augen-Prinzip vor der Übernahme eines MITRE-Imports nötig.
...
8. ✅ *(beantwortet, v10)* Gibt es eine Datenretention-/Löschpflicht für Engagement-Daten nach Projektabschluss? **Keine Löschpflicht in Breakpoint selbst** — die Rohdaten (Kundenschwachstellen-Detaildaten) liegen in anderen, vorgelagerten Case-Management-Systemen und unterliegen dort der jeweiligen Löschpflicht. Breakpoint selbst hält seine Daten (Engagements, Findings, abgeleitete Analysen, MITRE-Referenzdaten) **unbegrenzt** vor — keine automatische Löschung/Archivierung einzuplanen. *(Konsequenz für spätere Schritte: kein Retention-Job nötig; falls sich das je ändert, wäre ein expliziter, admin-gesteuerter Löschvorgang analog zum MITRE-Import-Rollback nachzuziehen, nicht automatisch im Hintergrund.)*
...
10. ✅ *(beantwortet und umgesetzt, v11)* MITRE hat "Defense Evasion" (TA0005) in **"Stealth"** umbenannt und **"Defense Impairment" (TA0112)** als neue 15. Taktik eingeführt. **Entscheidung: Breakpoint orientiert sich immer an der aktuellen MITRE-Taxonomie** — dauerhafte Leitplanke, nicht nur für diesen einen Fall. Umgesetzt über einen kuratierten Alias-Crosswalk statt FK-Kaskade (Abschnitt 10f) — stabile interne `tactic.id`, Anzeigename und neue Taktik werden über den ohnehin bei jedem Start laufenden idempotenten Re-Seed synchronisiert, keine Migration nötig.
